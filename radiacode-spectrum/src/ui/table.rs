...
impl TableLayout {
    pub fn cell_rect(&self, row_rect: Rect, column_index: usize) -> Rect {
        ...
        Rect::from_min_size(
            ...
            Vec2::new(column.width, row_rect.height()),
        )
    }
}
...
pub fn draw_table_header(ui: &mut Ui, layout: &TableLayout) {
    ...
    let labels: Vec<TableCellStyle> = layout
        ...
        .collect();
    paint_row_cells(ui, rect, layout, &labels);
}
...
pub fn draw_table_row(
    ...
    let (rect, response) =
        ui.allocate_exact_size(Vec2::new(layout.row_width, TABLE_ROW_ALLOC), sense);
    ...
    paint_row_cells(ui, rect, layout, cells);
    response
}

fn paint_row_cells(ui: &mut Ui, row_rect: Rect, layout: &TableLayout, cells: &[TableCellStyle]) {
    ...
    for (index, cell) in cells.iter().enumerate() {
        ...
        ui.painter()
            .text(pos, align, label, font.clone(), text_color);
    }
}
...
fn paint_intensity_cell(
    ...
    if track.width() > 0.0 {
        ui.painter()
            .rect_filled(track, 2.0, Color32::from_rgb(18, 22, 28));
